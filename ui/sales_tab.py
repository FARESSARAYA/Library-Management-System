...
class SalesTab:
    ...
    def load_materials(self):
        ...
        for mat in materials:
            ...
            if search.lower() in mat[1].lower() or search.lower() in barcode_str.lower():
                display_barcode = ' ' + barcode_str  # مسافة تمنع tkinter من حذف الأصفار
                item = self.tree.insert('', 'end', iid='bc_' + barcode_str,
                                        values=(display_barcode, mat[1], mat[2] if mat[2] else '-', mat[3], mat[4], mat[5]))
                if mat[5] <= 0:
    ...
    def setup_barcode_scanner(self):
        self._scan_timer = None  # مؤقت يُعاد ضبطه مع كل حرف
        ...
        def on_key_press(event):
            ...
            if event.widget == self.paid_entry:
                return
            if event.char and (event.char.isdigit() or event.char.isalpha() or event.char in '-_'):
                self.scan_buffer += event.char
                # إلغاء المؤقت السابق وإعادة ضبطه — يُطلق process_barcode بعد 200ms من آخر حرف
                if self._scan_timer:
                    self.parent.after_cancel(self._scan_timer)
                self._scan_timer = self.parent.after(200, self.process_barcode)

        def on_enter(event):
            if event.widget == self.quick_barcode:
                barcode = self.quick_barcode.get().strip()
                if barcode:
    ...
    def add_to_cart(self, event):
        ...
        barcode        = selected.replace('bc_', '', 1)  # من iid للحفاظ على الأصفار
    ...
    def edit_material(self):
        ...
        original_barcode = selected[0].replace('bc_', '', 1)  # من iid للحفاظ على الأصفار
