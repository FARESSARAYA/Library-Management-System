...
class Database:
    ...
    def add_sale(self, invoice_number, barcode, material_name, customer_name, unit_type, quantity, sell_price, total, discount_percent, discount_amount, date):
        """
        يحفظ سطر المبيع ويخصم الكمية داخل transaction واحدة.
        إذا فشل أي جزء يُتراجع عن الكل لضمان اتساق البيانات.
        """
        cursor = self.conn.cursor()
        try:
            ...
            cursor.execute(
                "UPDATE materials SET quantity = quantity - ? WHERE barcode=?",
                (quantity, barcode)
            )
            self.conn.commit()
            ...
            cursor.execute("SELECT quantity, name FROM materials WHERE barcode=?", (barcode,))
            result = cursor.fetchone()
            new_qty = result[0] if result else None
            name    = result[1] if result else material_name
