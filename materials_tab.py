...
# ══════════════════════════════════════════════════════════════════════
# دوال مساعدة للباركود
# ══════════════════════════════════════════════════════════════════════

def _clean_barcode(raw):
    """
    تنظيف قيمة الباركود القادمة من Excel أو أي مصدر آخر.

    المشكلة الأصلية:
      pandas يقرأ الأرقام من Excel كـ float64 تلقائياً.
      الباركود "0123456789" يُحوَّل إلى 123456789.0
      ثم str() تُعطي "123456789.0" — يضيع الصفر الأول والنقطة.

    الحل:
      1. القراءة الأصلية بـ dtype=str (في read_excel) تمنع التحويل.
      2. هذه الدالة تُنظّف أي حالة متبقية:
         - تزيل المسافات الزائدة
         - تزيل اللاحقة ".0" إن وُجدت (من تحويلات قديمة)
         - تتحقق أن القيمة ليست فارغة أو nan
    """
    if raw is None:
        return ""
    barcode = str(raw).strip()

    # إزالة اللاحقة العشرية .0 (ناتج تحويل float → str)
    # مثال: "123456.0" → "123456"  |  "0123456.0" → "0123456"
    if barcode.endswith(".0") and barcode[:-2].lstrip("-").isdigit():
        barcode = barcode[:-2]

    # تجاهل القيم الفارغة أو غير الصالحة
    if barcode.lower() in ("nan", "none", "null", ""):
        return ""

    return barcode


def _validate_barcode_input(new_value):
    """
    دالة التحقق من إدخال الباركود في حقل Entry.
    تقبل فقط الأرقام (بما فيها الصفر في البداية).
    تُستخدم مع validate='key' في tk.Entry.
    """
    # السماح بالحقل الفارغ أو الأرقام فقط
...
class MaterialsTab:
    ...
    def load_materials(self):
        ...
        for mat in materials:
            barcode = str(mat[0])   # ← نضمن أنه نص دائماً قبل إدخاله للـ Treeview
            if search.lower() in mat[1].lower() or search.lower() in barcode.lower():
                # ══════════════════════════════════════════════════════
                # الإصلاح: tkinter Treeview يُحوّل القيم التي تبدو
                # أرقاماً إلى integers عند تخزينها، فيضيع الصفر الأول.
                # الحل: نستخدم iid=barcode لحفظ الباركود الحقيقي
                # كمعرّف للصف، ونسترجعه بـ self.tree.focus() لاحقاً
                # بدلاً من values[0].
                # ══════════════════════════════════════════════════════
                self.tree.insert('', 'end', iid=barcode,
                                 values=(barcode, mat[1], mat[2] if mat[2] else '-',
                                         mat[3], mat[4], mat[5], mat[6]))

    def _make_barcode_entry(self, frame, row, col=1):
        """
        ينشئ حقل Entry للباركود مع:
          - التحقق من الإدخال (أرقام فقط، يحافظ على الصفر الأول)
          - منع أي تحويل تلقائي للنوع
        """
        vcmd = (self.parent.register(_validate_barcode_input), '%P')
        entry = tk.Entry(
            frame,
            font=('Arial', 11),
            width=25,
        ...
        entry.grid(row=row, column=col, padx=10, pady=8)
        return entry
    ...
    def add_material(self):
        ...
        for i, label in enumerate(labels):
            ...
            if label == '📦 الباركود':
                # ── حقل الباركود مع التحقق لمنع حذف الصفر ──
                entry = self._make_barcode_entry(frame, row=i)
            else:
        ...
        def save():
            # الباركود يُقرأ كنص خام بدون أي تحويل
    ...
    def edit_material(self):
        ...
        # ══════════════════════════════════════════════════════════════
        # الإصلاح: نقرأ الباركود من iid الصف (المعرّف الذي ضبطناه
        # في load_materials) وليس من values[0].
        # السبب: tkinter يُحوّل values[0] = "0123456" إلى 123456
        # (integer) عند الاسترجاع، بينما iid يبقى نصاً كما خُزِّن.
        # ══════════════════════════════════════════════════════════════
        original_barcode = selected[0]                     # iid = الباركود الأصلي كنص
        values           = self.tree.item(selected[0])['values']

        # نبني قاموساً يربط كل حقل بقيمته الصحيحة
        field_values = {
            '📦 الباركود':         original_barcode,       # ← من iid وليس values[0]
            '📝 اسم المادة':        str(values[1]),
            '🏢 اسم التاجر':        str(values[2]) if values[2] != '-' else '',
            '📏 الوحدة':            str(values[3]),
            '💰 السعر':             str(values[4]),
            '📦 الكمية':            str(values[5]),
            '⚠️ الحد الأدنى':       str(values[6]),
        }
        ...
        for i, label in enumerate(labels):
            ...
            # نملأ الحقل من field_values وليس من values[i] مباشرة
            entry.insert(0, field_values[label])
            entries[label] = entry
        ...
        def save():
            ...
            # original_barcode من iid — مضمون بأصفاره
    ...
    def delete_material(self):
        ...
        # الباركود الأصلي بأصفاره محفوظ في iid
        original_barcode = selected[0]
        values           = self.tree.item(selected[0])['values']

        if messagebox.askyesno("🗑️ تأكيد الحذف", f"هل أنت متأكد من حذف المادة:\n\n📝 {values[1]}"):
            self.db.delete_material(original_barcode)   # ← iid وليس values[0]
            self.load_materials()
            self.notify_change()
    ...
    def import_from_excel(self):
        ...
        try:
            # ══════════════════════════════════════════════════════════
            # الإصلاح الجوهري: قراءة جميع الأعمدة كنصوص (dtype=str)
            #
            # السبب: pandas يُحوّل الباركود تلقائياً إلى float64:
            #   "0123456789" → 123456789.0 → str → "123456789.0"
            #   الصفر الأول يضيع والنقطة تُضاف!
            #
            # بـ dtype=str: كل خلية تُقرأ كنص كما هي في الملف تماماً.
            # ══════════════════════════════════════════════════════════
            ...
            for idx, row in df.iterrows():
                # ── تنظيف الباركود مع الحفاظ على الأصفار ──
                barcode = _clean_barcode(row[barcode_col])
                name    = str(row[name_col]).strip()

                if not barcode or not name or name.lower() in ('nan', 'none', ''):
                    skipped += 1
                    continue
    ...
    def import_price_from_excel(self):
        ...
        try:
            # ── قراءة كنصوص لمنع حذف الأصفار من الباركود ──
            if file_path.endswith('.csv'):
                df = pd.read_csv(file_path, dtype=str, encoding='utf-8-sig')
            else:
                df = pd.read_excel(file_path, dtype=str)
            ...
            for idx, row in df.iterrows():
                # تنظيف الباركود مع الحفاظ على الأصفار
                raw_bc = _clean_barcode(row[barcode_col])
                if not raw_bc:
                    continue
